...
def fetch_latest_player():
    conn = sqlite3.connect('players.db')
    cursor = conn.cursor()
    cursor.execute('SELECT player_num from players ORDER BY player_num DESC LIMIT 1')
    player = cursor.fetchone()
    conn.close()
    if player:
        player = player[0]
...
@app.register_rpc
async def register_players() -> RegisterPlayers:  # it'll send true if player successfully registered else false
    players_count = fetch_player_count()
    latest_player = fetch_latest_player()
    if players_count < 2:
        if players_count == 0:
            player = 1
        else:
            player = 2 if latest_player == 1 else 1
...
async def move(player_move: dict) -> MoveStatus:  # function to receive move from client
    ...
    if move_played_by == CURR_PLAYER_NO:  # check if move is initiated by current player
        ...
        if _move_text == "Invalid move":
            return MoveStatus(status="Failed", reason="Invalid move: Cell already occupied")
